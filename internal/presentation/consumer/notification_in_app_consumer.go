...
func (c *NotificationInAppConsumer) Handle(ctx context.Context, body []byte) error {
	...
	if err := json.Unmarshal(body, &msg); err != nil {
		...
		if errUnified := json.Unmarshal(body, &unifiedMsg); errUnified == nil && unifiedMsg.UserID != uuid.Nil {
			...
			msg = consumers.InAppNotificationMessage{
				...
				Message:        unifiedMsg.Body, // Map Body to Message
				Type:           unifiedMsg.Type,
				Data:           unifiedMsg.Data,
				CreatedAt:      unifiedMsg.CreatedAt,
			}
			if msg.Type == "" {
				msg.Type = "info" // Default type
			}
		} else {
	...
	if err := c.notificationRepository.UpdateDeliveryAttempt(ctx, msg.NotificationID, attempt); err != nil {
	...
	if err := c.notificationRepository.UpdateStatus(ctx, msg.NotificationID, enum.NotificationStatusSent); err != nil {
