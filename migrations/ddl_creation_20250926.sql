...
-- General financial terms that can apply to multiple types
compensation_amount DECIMAL (15, 2), -- The total fixed compensation for the contract work. Used mainly in 'ADVERTISING' or as a base for 'BRAND_AMBASSADOR'.
payment_terms TEXT, -- A brief summary of payment schedules or conditions, e.g., "Net 30", "50% upfront, 50% on completion".

-- Advertising-Specific Fields (Used when type is 'ADVERTISING')
deliverables JSONB, -- A structured list of content to be produced. Example: '[{"type": "video", "count": 2}, {"type": "post", "count": 5}]'.
usage_rights TEXT, -- A summary of how the brand can use the content, e.g., "12 months for digital channels".

-- Affiliate-Specific Fields (Used when type is 'AFFILIATE')
-- The percentage of a sale that is paid to the affiliate.
commission_rate DECIMAL (5, 2),
cookie_duration_days INTEGER, -- The number of days a tracking cookie remains active after a user clicks the affiliate's link.
payout_threshold DECIMAL (15, 2), -- The minimum amount of commission an affiliate must earn before a payment is issued.

-- Brand Ambassador-Specific Fields (Used when type is 'BRAND_AMBASSADOR')
-- A recurring monthly payment for the ambassador's services.
monthly_retainer DECIMAL (15, 2),
required_posts_per_month INTEGER, -- The minimum number of promotional posts required from the ambassador each month.

-- Co-Producing-Specific Fields (Used when type is 'CO_PRODUCING')
revenue_share_percentage DECIMAL (5, 2), -- The percentage of revenue from the co-produced product that goes to the partner.
ip_ownership TEXT, -- A brief summary of who owns the intellectual property of the final product, e.g., "Jointly owned by Brand and Influencer".

-- For extensions or renewals of previous contracts.
...
-- Product story to hold rich content about the product, this is reserved for limited_product only
...
-- NOTE: You must first enable the extension in your database by running: CREATE EXTENSION IF NOT EXISTS pg_trgm;
